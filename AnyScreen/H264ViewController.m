...
//            });
            ...
            //[[CSScreenRecorder sharedCSScreenRecorder].simpleSession sendVideoData:[NSData dataWithBytes:vp.buffer length:vp.size] length:vp.size];
            [[CSScreenRecorder sharedCSScreenRecorder].simpleSession sendPixelBuffer:pixelBuffer];
...
-(IBAction)on_playButton_clicked:(id)sender {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [self decodeFile:self.fileName fileExt:@"h264"];
    });
}
- (void)open {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
